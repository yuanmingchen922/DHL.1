...
X_{ij}\le MY_i, Z_{jc}\le MY_j. In this version (Ver_3), I introducted Lagrangian Relaxation. The constraints are soften. It's no longer cosidered as hard constraints in the inner problem. I multiple λij, and μjc as stated below: 
\max\sum_{i,j}\lambda_{ij}(X_{ij}-U_{ij}Y_i)+\sum_{j,c}\mu_{jc}(Z_{jc}-V_{jc}Y_j)
Then the max function penalized any violation of these constraints, making inner problem easier to solve. Approaches through using fewer constraints and separable structure

# The following parts shows mathematical proof for serval parts that needs to be satisfied in order to find the optimum for all feasible solitions

All variables and constriants are stated ealier before. Denote hard constraints 1-3 as H and take linking constraint in Lagrange to be relaxed denote LK

Proof: The following consists several parts 
Firstly, define the Lagrangan Function and dual function
